...
void AR_load_initial_settings(void)
{
  ...
  add_application_(AR_CSRV_ADC, ADC_update);
  add_application_(AR_CSRV_RAMECC, RAMECC_update);
  add_application_(AR_CSRV_GPIO, GPIO_update);
  add_application_(AR_CSRV_THERMOMETER, THERMOMETER_update);
  add_application_(AR_CSRV_IFLASH, IFLASH_update);
